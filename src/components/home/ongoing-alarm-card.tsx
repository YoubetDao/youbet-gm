...
function OngoingAlarmCard({ props }: AlarmProps) {
  ...
  const clickConfirm = async () => {
    await sdk.contract.confirmTaskCompletion(props.id!, address!)
  }
  const stake = async () => {
  ...
  const renderContent = () => {
    switch (status) {
      case AlarmStatus.Confirm:
        return (
          <Button variant={'default'} onClick={clickConfirm} className="bg-green-300 w-12 h-10">
            Confirm!
          </Button>
        )
      case AlarmStatus.Pending:
        return (
          <Button className="text-black bg-green-600 w-12 h-10" variant={'default'} onClick={stake}>
            Stake
          </Button>
        )
      case AlarmStatus.InProgress:
        return <Button variant={'destructive'}>Lock</Button>
      default:
        return <Button variant={'destructive'}>Lock</Button>
    }
  }
